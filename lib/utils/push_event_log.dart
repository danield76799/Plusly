...
class PushEventLog {
  ...
  Future<void> _persist() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      // RELOAD IS VERPLICHT vóór het lezen. SharedPreferences.getInstance()
      // geeft een PER-ISOLATE cache (static Completer<SharedPreferences> in
      // shared_preferences_legacy.dart). Zonder reload lees je je eigen
      // verouderde beeld en schrijf je dat terug — waarmee je de regels van
      // het andere isolate alsnog overschrijft. De merge hieronder is dan
      // schijnveiligheid: hij voegt samen met een stale kopie.
      await prefs.reload();
  ...
  Future<void> load() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      // Ook hier reloaden: bij een verse app-start moet dit de geschiedenis
      // van de VORIGE sessie zien, niet de lege cache van dit isolate.
      await prefs.reload();

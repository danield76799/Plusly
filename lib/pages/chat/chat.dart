...
import 'package:Pulsly/utils/clipboard_utils.dart';
...
class ChatController extends State<ChatPageWithRoom>
  ...
  void updateView() {
    if (!mounted) return;
    Logs().d('[EchoDiag] updateView() — setState fires; timeline.events.length = ${timeline?.events.length ?? "null"}');
    setState(() {});
  }
  ...
  Future<void> send() async {
    ...
    Logs().d('[EchoDiag] send() pre-send — timeline.events.length=${timeline?.events.length ?? "null"}, txid=$txid');
    ...
    sendFuture.then((_) {
      Logs().d('[EchoDiag] sendFuture resolved — timeline.events.length=${timeline?.events.length ?? "null"}, txid=$txid');
    }, onError: (e) {

...
release tag / txt     = X + 2001   (e.g. 21008)
APK versionCode       = X + 2001   (e.g. 21008)   ← Flutter adds +2001 internally
```

Key facts:
- Flutter's `flutter build --build-number=Y` results in an APK `versionCode`
  of **Y + 2001** on the CI runner. This +2001 offset is applied by Flutter
...
- The CI passes the **pubspec build number** (`build_number` output of the
  ...
  `versionCode = X + 2001`, which equals the release tag automatically.
...
Because Flutter also adds +2001, the APK ended up at
`NEW_BUILD + 2001 + 2001` = `NEW_BUILD + 4002`, while the tag was only
`NEW_BUILD + 2001`. The app then reported a version one (or two) higher than
the release, causing a permanent "update available" loop where users could
never satisfy the updater.

The fix: `--build-number` receives the **raw pubspec build** (no +2001), and
Flutter's own +2001 makes the APK match the tag/txt exactly.
...
2. The release tag and `plusly-version.txt` MUST use
   `android_build_number` (= `build_number + 2001`).
3. Never pass `android_build_number` to `--build-number` — that double-counts
   the +2001 offset and reintroduces the update loop.
4. If you change the offset math, bump the pubspec build high enough that the
